...
public class Tyuana2T extends UserBase {
	private Logger logger = LoggerFactory.getLogger(Tyuana2T.class);
	
	int bettingSortTime;
	MartinKumiban martinCalculator = null;
	DistributeAmount distamo = null; 

	public Tyuana2T(String fromYmd, String toYmd, BettingRule rule) throws Exception{
		super();
...
//		String[] oddsRankNormalList = rule.getString("oddsRankNormalList").split(",");
//		String[] kumibanNormalList = new String[2];
//		
//		for (int i = 0; i < oddsRankNormalList.length; i++) {
//			OddsTItem odds = OddsManager.getInstance().getRankedOddsItem(race.getJyoCd(), race.getRaceNo(), betType, Integer.parseInt(oddsRankNormalList[i]));
//			if (odds != null) {
//				race.betlist.add(new Bet(betType, odds.value, rule.getInt("distamoStartAmount") * rule.getInt("distamoMultiplier"), 0, odds.kumiban));
//				kumibanNormalList[i] = odds.kumiban; 
//			}
//		}
//		if (race.betlist.size() > 0) {
//			float virtualOdds = getVirtualOdds(race, kumibanNormalList);
...
//		}
	...
	public static void main(String[] args) {
		try {
			...
			BoatProperty.init("C:/Dev/workspace/Oxygen/pod_boatrace/test/properties/tyuana_1.properties");
			BettingRule rule = new BettingRule(BoatProperty.DIRECTORY_PROPERTY + "tyuana_1.properties");
			Honmyo2T user = new Honmyo2T("20170309", "20180505", rule);
//			Honmyo2T user = new Honmyo2T("20170309", "20170930", rule);
			user.start();
		} catch (Exception e) {
			e.printStackTrace();
		}

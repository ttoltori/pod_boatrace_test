...
public class User_11 extends UserBase_11 {
	private Logger logger = LoggerFactory.getLogger(User_11.class);
	
	int bettingSortTime;
	boolean isMartin = false;

	public User_11(String fromYmd, String toYmd, BettingRule rule) {
		super();
	...
	protected RaceEx createAndSetBetList(RaceEx race) throws Exception{
		...
		for (String kumiban : kumibanList) {
			...
			if (odds == null || odds < rule.getFloat("minimumOdds")) {
				race.betlist.clear();
				return race;
			} else {
				Bet bet = new Bet(rule.getString("type"), odds, rule.getInt("baseBetAmount"), 0, kumiban);
	...
	protected RaceEx setPopularLevel(RaceEx race) throws Exception {
		...
		Waku subWaku1 = race.wakuList.get(2);
		Waku subWaku2 = race.wakuList.get(3);
		if (
				( mainWaku.nationWiningRate > 5)
				&& ( subWaku1.nationWiningRate > 5 || subWaku2.nationWiningRate > 5)
			) {
	...
	public static void main(String[] args) {
		try {
			...
			BoatProperty.init("C:/Dev/workspace/Oxygen/pod_boatrace/test/properties/rule_11.properties");
			BettingRule rule = new BettingRule(BoatProperty.DIRECTORY_PROPERTY + "rule_11.properties");
			User_11 user = new User_11("20170309", "20180430", rule);
			user.start();
		} catch (Exception e) {
			e.printStackTrace();
		}

...
public class DistributeAmount {
	...
	public DistributeAmount(int strategy, float factor1, int baseAmount) {
	...
	public List<DistamoItem> distribute(int startAmount, List<OddsTItem> oddsList, int maxAmount) {
	...
	private boolean isPrizeOk(int[] moneys, List<OddsTItem> oddsList) {
		int[] prizes = new int[moneys.length];
		
		int betSum = Arrays.stream(moneys).sum();
		int plusCount = 0;
		for (int i = 0; i < prizes.length; i++) {
			prizes[i] = MathUtil.ceilAmount((float)moneys[i] * oddsList.get(i).value, baseAmount);
